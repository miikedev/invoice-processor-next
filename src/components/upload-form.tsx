...
export default function UploadForm({userId} : {userId: string}) {
    console.log('userId in upload form', userId)
    ...
    return (
        <>
            ...
            <form
                ...
                className='flex flex-col gap-3 max-h-[20rem] min-w-[8rem] w-84'
            >
                <Input name="file" ref={inputFileRef} type="file" accept="image/jpeg, image/png, image/webp, image/jpg" required />
                <Button type="submit" disabled={isLoading}>
                    {isLoading ? <div className='flex items-center gap-3'>Processing <Loader className='animate-spin' /></div> : 'Submit'}
                </Button>
            </form>
            {url && (
                <div className='flex flex-col gap-5'>
                    <span className='font-semibold'>Blob url:</span> <Link href={url} target='_blank' className=''>{url}</Link>
